...
./safety_protocol.py lint /path/to/risk-note.yaml --schema risk_note.schema.json

# 4) Check evidence freshness (fails CI if stale)
./safety_protocol.py check-evidence /path/to/risk-note.yaml --max-age P30D

# 5) Generate a lightweight GSN-style view
./safety_protocol.py generate-assurance-case /path/to/risk-note.yaml > case.gsn.txt

# 6) Rank investigations by EVOI per hour within a budget
./safety_protocol.py prioritize ./notes --budget PT40H

# 7) Enforce posture at release time
./safety_protocol.py gate-check /path/to/risk-note.yaml --posture green
